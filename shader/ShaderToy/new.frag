...
const float N = 1.33;
const float zoom = 1.5707963267948966192313216916398;
const int max_intersections = 12;

const float min_distance = 1.0;
const float max_distance = 20.5;
...
vec4 background(vec3 d) {
  return texture(environmentMap, vec3(d.x, -1. * d.y, d.z));
}

float f(vec3 p) {
  return 1.0 -
         (sphere1.w / sq(sphere1.xyz - p) + sphere2.w / sq(sphere2.xyz - p) +
...
vec3 fd(vec3 p) {
  ...
  return 2.0 * (sphere1.w * d1 / sq(sq(d1)) + sphere2.w * d2 / sq(sq(d2)) +
                sphere3.w * d3 / sq(sq(d3)));
}

float stepsize(vec3 p) {
  float md = sqrt(
      min(min(sq(p - sphere1.xyz), sq(p - sphere2.xyz)), sq(p - sphere3.xyz)));
    ...
    for (int i = 0; i < maxsteps; ++i) {
      ...
      if (f(p + d * (k + ss)) * nf < 0.0) {
        ...
        if (r == vec3(0.0)) {
          ...
          float f = nf > 0.0 ? fresnel(1.0, N, dot(-d, n))
...
void main() {
  float t = iTime;
  vec4 vs1 = cos(t * vec4(0.87, 1.13, 1.2, 1.0) + vec4(0.0, 3.32, 0.97, 2.85)) *
             vec4(-1.7, 2.1, 2.37, -1.9);
  vec4 vs2 =
      cos(t * vec4(1.07, 0.93, 1.1, 0.81) + vec4(0.3, 3.02, 1.15, 2.97)) *
      vec4(1.77, -1.81, 1.47, 1.9);
  ...
  vec2 uv = zoom * (iResolution.xy * fragCoord.xy - iResolution.xy * 0.5) /
            iResolution.x;
  ...
  color += ray(
      eye, normalize(forward + (uv.y + uvh.y) * up + (uv.x + uvh.x) * left));
#if MULTISAMPLES > 2
  ...
  if (color.a <= 3)
    discard;
  // gl_FragDepth=200./(101.-99.*color.a);
  float mapped_depth = (1 / color.a - 1) / (0.01 - 1);
  gl_FragDepth = mapped_depth;
  // FragColor=vec4(vec3(mapped_depth), 1.0);
}
