...
// - Ground: Basic (no reflection, no refraction), roughness and density varying
// foreach ball.
// - Along blue wall: Reflective materials, roughness and density varying
// foreach ball.
// - Along orange wall: Refractive materials,roughness and density varying
// foreach ball.
...
struct Material {
  ...
  float density;    // Cook-Torrance color density i.e. fraction of diffuse
                    // reflection
};
  ...
  if (d > 0.) {
    ...
    if (t2 > 0.) {
      ...
      if ((dist < i.d) || (i.d < 0.)) {
        ...
        i.inside = t1 < 0.;
        if (i.inside)
          i.n *= -1.;  // invert the normal when hitting inside during
                       // refraction
        i.mat = mat;
      }
    }
...
vec3 shadeCookTorrance(Inter i, Light lig) {
  ...
  vec3 spec = (NdotV * NdotL == 0.)
                  ? vec3(0.)
                  : vec3(fres * geo * rough) / (NdotV * NdotL);
  vec3 res = NdotL * ((1. - K) * spec + K * i.mat.color) *
             lig.color;  // * exp(-0.001*length(lig.pos-i.p));
  return res;
}
...
vec3 raytraceRay(vec3 ro, vec3 rd, inout Inter i) {
  ...
  if (intPlane(ro, rd, p, n, mat, i)) {
    // checker plane hack
    i.mat.color =
        vec3(0.95, 0.35, 0.) * mod(floor(i.p.y) + floor(i.p.z), 2.) + 0.25;
  }
  //
  p = vec3(0., 0., 8.);
  n = vec3(0., 0., 1.);
  if (intPlane(ro, rd, p, n, mat, i)) {
    // checker plane hack
    i.mat.color =
        vec3(0.35, 0.65, 0.95) * mod(floor(i.p.x) + floor(i.p.y), 2.) + 0.25;
  }
  //
